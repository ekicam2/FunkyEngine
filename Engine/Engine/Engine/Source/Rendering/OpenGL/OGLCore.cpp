...
namespace Funky
{
	namespace Rendering
	{
		...
		void OGL::ClearRenderTargetWithColor(Math::Vector3f const & Color, RenderingBackend::RenderTarget RenderTargetToClear)
		{
			...
			if (RenderTargetToClear == RenderingBackend::INVALID_INDEX)
			{
		...
		void OGL::DrawMesh(RenderingBackend::MeshProxy Mesh)
		{
			glBindVertexArray(Meshes[Mesh].VAO);
			glDrawElementsBaseVertex(GL_TRIANGLES, (GLsizei)Meshes[Mesh].IndicesCount, GL_UNSIGNED_INT, (void*)0, 0);
			glBindVertexArray(0);
		}
