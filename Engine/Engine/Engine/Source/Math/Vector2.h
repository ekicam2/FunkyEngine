...
namespace Funky
{
	namespace Math
	{
		...
		using Vector2f = Vector2D<f32>; //!< Template instance for 2 dimensional float vector 
		using Vector2d = Vector2D<f64>; //!< Template instance for 2 dimensional double vector 
		using Vector2u = Vector2D<u32>; //!< Template instance for 2 dimensional unsigned vector 
	}
}
...
//	static_assert(																\
//		   offsetof(Funky::Math::Vector2D<in_type>, X) == 0 * sizeof(in_type) 	\
//		&& offsetof(Funky::Math::Vector2D<in_type>, Y) == 1 * sizeof(in_type) 	\
